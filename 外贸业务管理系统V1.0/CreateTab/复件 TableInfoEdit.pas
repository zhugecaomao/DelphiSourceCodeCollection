...
type
  TTableInfoEdit_frm = class(TBaseFrm)
    ADOQuery1: TADOQuery;
    suiForm1: TsuiForm;
...
var
  TableInfoEdit_frm: TTableInfoEdit_frm;

implementation

uses DataModule, BasePage, CreateTabInfo, TabInfoEdit;

{$R *.dfm}

procedure TTableInfoEdit_frm.ClearPanel(Sender: TObject);
begin
...
//清空控件内容
procedure TTableInfoEdit_frm.Control_Clear(Sender:TObject);
var i:integer;
begin
...
//清空控件内容
procedure TTableInfoEdit_frm.Control_ReadOnly(Sender:TObject);
var i:integer;
begin
   for i:=0 to TWinControl(Sender).ControlCount -1 do
   begin
...
procedure TTableInfoEdit_frm.Set_TableHead();
var sqlstr:string;
begin
   with ADOQuery1 do
   begin
...
procedure TTableInfoEdit_frm.Create_frame();
var sqlstr :string;
    Component:TObject;
...
procedure TTableInfoEdit_frm.Set_FrameValue();
...
procedure TTableInfoEdit_frm.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  Case Key of
...
procedure TTableInfoEdit_frm.bbsaveClick(Sender: TObject);
begin
   //UpdateTableInfo(nil);
   try
     Query.Post;
...
//保存表
procedure TTableInfoEdit_frm.UpdateTableInfo(Sender: TObject);
...
procedure TTableInfoEdit_frm.FormShow(Sender: TObject);
var sqlstr:string;
begin
   Set_TableHead();
   Set_FrameValue();
  ...
  begin 
     ...
     Query.FieldByName(TableInfoEdit_frm.KeyValue[0, 0]).AsString :='';
  end;
...
procedure TTableInfoEdit_frm.FormCreate(Sender: TObject);
begin
  self.Caption :='';
  SetLength(TableInfoEdit_frm.KeyID, 1);

end;

procedure TTableInfoEdit_frm.bbcloseClick(Sender: TObject);
begin
  close();
end;

procedure TTableInfoEdit_frm.bbclearClick(Sender: TObject);
begin
  Control_Clear(ParentPanel);
end;
