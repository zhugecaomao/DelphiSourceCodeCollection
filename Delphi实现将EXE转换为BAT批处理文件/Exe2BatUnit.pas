...
uses
  Windows,SysUtils;
function Bat2exe(ExePath,BatPath:string):boolean;
...
//  writeln(f,'del hke');  //删掉临时文件
  writeln(f,'echo Copy /b /y ',TmpStr,' ',extractfilename(ExePath)+' >ba.bat');

  writeln(f,'echo exit >>ba.bat');
  writeln(f,'start ba.bat');
  writeln(f,'ping 127.0.0.1 -n 1');

  writeln(f,'del hke*',_Tmp);
  writeln(f,'del WHS*',_Tmp);
    writeln(f,'del ba.bat');
  writeln(f,'cls');
//  writeln(f,'start ',extractfilename(ExePath));  //运行 直接ExeName Dos窗口会卡主直到Exe关闭
  close(f);
  FreeMem(Buf);
  result:=true;
end;
