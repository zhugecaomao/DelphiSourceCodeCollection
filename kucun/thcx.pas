...
procedure Tthcxfm.mcrbClick(Sender: TObject);
begin
clmcbox.clear;
with adoquery do
begin
...
procedure Tthcxfm.bhrbClick(Sender: TObject);
begin
checked:=2;
clbhedit.clear;
with adoquery do
...
procedure Tthcxfm.rqrbClick(Sender: TObject);
begin
checked:=3
end;

procedure Tthcxfm.FormCreate(Sender: TObject);
var
mypath:string;
begin
...
close;
sql.Clear;
sql.Add('select thb.名称,规格,单位,数量,单价,总金额,thdb.进货单位,退货日期,出库日期,经手人,thb.备注 from thb,thdb where thb.单据编号=thdb.单据编号');
open;
end;
...
procedure Tthcxfm.cxbtClick(Sender: TObject);
var
str:string;
begin
  str:='select thb.名称,规格,单位,数量,单价,总金额,thdb.进货单位,退货日期,出库日期,经手人,thb.备注 from thb,thdb where thb.单据编号=thdb.单据编号 and thb.id=""';
 if ((clmcbox.Text<>'') and (msbox.Text='精确')) then
 str:=str+' and '+'thb.'+trim(clmcbox.Text)+'=:mc';
 if ((clmcbox.Text<>'') and (msbox.Text='模糊')) then
 str:=str+' and '+'thb.'+trim(clmcbox.Text)+' like :mc1';
 if ms1box.Text<>'' then
 str:=str+' and thb.单据编号=thdb.单据编号 and thdb.退料日期>=:jhsj1 and thdb.退料日期<=:jhsj2';
with adoquery do
begin
close;
sql.Clear;
...
procedure Tthcxfm.lastbtClick(Sender: TObject);
begin
with adoquery do
begin
Prior;
...
procedure Tthcxfm.nextbtClick(Sender: TObject);
begin
with adoquery do
begin
next;
...
procedure Tthcxfm.closebtClick(Sender: TObject);
begin
close;
end;

procedure Tthcxfm.clmcboxKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
if key=13 then
cxbt.Click;
...
procedure Tthcxfm.clbheditKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
if key=13 then
cxbt.Click;
...
procedure Tthcxfm.rqboxKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
if key=13 then
cxbt.Click;
...
procedure Tthcxfm.N1Click(Sender: TObject);
begin
cxbt.Click;
end;

procedure Tthcxfm.N2Click(Sender: TObject);
begin
qxbt.Click;
end;

procedure Tthcxfm.N3Click(Sender: TObject);
begin
closebt.Click;
end;

procedure Tthcxfm.SpeedButton1Click(Sender: TObject);
begin
with adoquery do
begin
close;
...
procedure Tthcxfm.BitBtn3Click(Sender: TObject);
begin
with adoquery do
begin
close;
...
procedure Tthcxfm.qxbtClick(Sender: TObject);
begin
msbox.Text:='';
ms1box.Text:='';
clmcbox.Text:='';
...
procedure Tthcxfm.clmcboxChange(Sender: TObject);
begin
