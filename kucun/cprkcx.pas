...
close;
sql.Clear;
sql.Add('select cprkb.名称,规格,单位,数量,cprkdb.经手人,入库日期,cprkb.备注 from cprkb,cprkdb where cprkdb.单据编号=cprkb.单据编号');
open;
end;
...
str:string;
begin
  str:='select cprkb.名称,规格,单位,数量,cprkdb.经手人,入库日期,cprkb.备注 from cprkb,cprkdb where cprkdb.单据编号=cprkb.单据编号 and  cprkb.id=""';
 ...
 if ms1box.Text<>'' then
 str:=str+' and cprkb.单据编号=cprkdb.单据编号 and cprkdb.入库日期>=:jhsj1 and cprkdb.入库日期<=:jhsj2';
with adoquery do
begin
close;
sql.Clear;
