...
{ THotLink }
  ...
  published
    ...
    {* 超链接内容或文件名，例如：
     |<PRE>
       http://yygw.126.com      - 网页
       mailto:yygw@163.com      - 邮件地址
       mailto:yygw@163.com?subject=你好 - 带邮件标题的邮件地址链接
...
{ TAAScrollText }
  ...
  published
    ...
    property ScrollQuality: TTimerQuality read GetScrollQuality write SetScrollQuality
      default tqHigh;
    {* 滚动精度，如果滚动较慢，可考虑设置为tqLow以节约系统资源}
...
const
  ...
  '<Title2>控件作者'#13#10 +
    '<Text1>作者：周劲羽'#13#10 +
    'Email：yygw@163.com'#13#10 +
    'Http://yygw.126.com'#13#10 +
    '与月共舞工作室'#13#10;
  ...
  csDanceWithMoon =
    '<Center><Title4>与 月 共 舞'#13#10 +
    '<Text2>《仙剑奇侠传》插曲'#13#10 +
    '原曲：《蝶恋》'#13#10 +
    '填词：与月共舞'#13#10#13#10 +

  '<Text3>漫不经心唤出你的名字'#13#10 +
    '恍惚间已是光阴纵逝'#13#10#13#10 +
...
begin
  ...
  ScrollTimer := TTimerEx.Create(Self);
...
//取滚动精度
function TAAScrollText.GetScrollQuality: TTimerQuality;
begin
  Result := ScrollTimer.Quality;
end;
...
begin
  ...
  begin
    FScrollStep := Value;
    if FScrollStep <= 0 then
      FScrollStep := 1;
  end;
end;

//设置滚动精度
procedure TAAScrollText.SetScrollQuality(const Value: TTimerQuality);
begin
  ScrollTimer.Quality := Value;
end;
...
begin
  ...
  begin
    FCurrPos := Value;
    if (FCurrPos < 0) or (FCurrPos > TextBmp.Height) then
      FCurrPos := 0;
    Paint;
  end;
end;
...
begin
  Result := (Lines.Text <> csAAFadeTextCopyRight) and (Lines.Text <>
    csDanceWithMoon);
end;
