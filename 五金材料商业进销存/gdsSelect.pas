...
uses
  ...
  Menus, wwDBGridEx, ActnList, XPMenu, Variants, wwcheckbox;

type
  TfrmGdsSelect = class(TForm)
    ...
    procedure lvSelectedDblClick(Sender: TObject);
...
procedure TfrmGdsSelect.LoadTree();
var Level: integer;
  preNode: TfcTreeNode;
  CurNode: TfcTreeNode;
  tmpstr: string;
...
procedure TfrmGdsSelect.FormShow(Sender: TObject);
begin
  if not dm.tabtype.active then dm.tabtype.open;
  Dm.tabType.First;
  LoadTree();
...
function TfrmGdsSelect.NextID(current: string): string;
...
procedure TfrmGdsSelect.TreeView1Editing(TreeView: TfcCustomTreeView;
...
procedure TfrmGdsSelect.TreeView1Edited(TreeView: TfcCustomTreeView;
...
procedure TfrmGdsSelect.FormCreate(Sender: TObject);
...
begin
  ...
  with lvSelected do
    begin
      ...
      NewColumn.Caption := '商品编号';
      NewColumn.width := 100;
      NewColumn := Columns.Add;
      NewColumn.Caption := '商品名称';
      NewColumn.width := 200;
      NewColumn := Columns.Add;
      NewColumn.Caption := '单位';
      NewColumn.width := 50;
      NewColumn := Columns.Add;
      NewColumn.Caption := '库存数量';
      NewColumn.width := 100;
      NewColumn := Columns.Add;
      NewColumn.Caption := '库存金额';
      NewColumn.width := 100;
    end;
  UpdateGoodsList(Treeview1.TopItem);
end;

procedure TfrmGdsSelect.UpdateGoodsList(Node: TfcTreeNode);
var tmpbegin: string;
  tmpend: string;
  ListItem: TListItem;
begin
...
procedure TfrmGdsSelect.Treeview1Change(TreeView: TfcCustomTreeView;
  Node: TfcTreeNode);
begin
  UpdateGoodsList(Node);
end;

procedure TfrmGdsSelect.btnSearchClick(Sender: TObject);
begin
...
procedure TfrmGdsSelect.rbShortKeyPress(Sender: TObject; var Key: Char);
begin
  btnSearchClick(Sender);
end;

procedure TfrmGdsSelect.edSearchKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TfrmGdsSelect.actDeleTypeExecute(Sender: TObject);
var tmpSQL: TADOQuery;
  isall: boolean;
  tmpbegin: string;
  tmpend: string;
...
procedure TfrmGdsSelect.actModifyTypeExecute(Sender: TObject);
var
  NewString: string;
  ClickedOK: Boolean;
  tmpSql: TADOQuery;
...
procedure TfrmGdsSelect.actNewTypeExecute(Sender: TObject);
...
procedure TfrmGdsSelect.actSetFirstExecute(Sender: TObject);
...
procedure TfrmGdsSelect.actGdsDeleteExecute(Sender: TObject);
begin
...
procedure TfrmGdsSelect.actGdsAddExecute(Sender: TObject);
begin
  OpenForm(Tfrmgoods, frmgoods, self);
end;

procedure TfrmGdsSelect.wwDBGridEx1DblClick(Sender: TObject);
var ListItem: TListItem;
begin
  with qryFilter, lvSelected do
    begin
      if fieldbyname('gdsID').asstring = '' then exit;
      if FindCaption(0, fieldbyname('gdsID').asstring, false, true, false) <> nil then exit; //有相的相同记录存在。
      disablecontrols;
      Items.BeginUpdate;
      ListItem := lvSelected.Items.Add;
      if lvSelected.Items.Count <= lvSelected.SmallImages.Count then
        ListItem.ImageIndex := lvSelected.Items.Count
      else ListItem.ImageIndex := 0;
      ListItem.Caption := fieldbyname('gdsID').asstring;
      ListItem.SubItems.Add(fieldbyname('gdsname').asstring);
      ListItem.SubItems.Add(fieldbyname('unit').asstring);
      ListItem.SubItems.Add(fieldbyname('amoqua').asstring);
      ListItem.SubItems.Add(fieldbyname('amoamo').asstring);
      Items.EndUpdate;
      enablecontrols;
    end; //end of with
end;

procedure TfrmGdsSelect.wwDBGridEx1RowChanged(Sender: TObject);
begin
  with qryGdsStore do
    begin
      close;
...
procedure TfrmGdsSelect.dbgStoreDblClick(Sender: TObject);
begin
  actSetFirstExecute(Sender);
end;

procedure TfrmGdsSelect.labNoRecordDblClick(Sender: TObject);
begin
  actSetFirstExecute(Sender);
end;

procedure TfrmGdsSelect.lvSelectedDblClick(Sender: TObject);
begin
  if lvSelected <> nil then lvSelected.Selected.Delete;
end;
