...
begin
  ...
  iMTax := 0;
  iMRecTotal := 0;
  QryExec(Qry1,'select F04,Sum(F08) as STaxtotal,Sum(F10) as SRectotal from T2050 where F13=''进'' and F03='''+sCNo+''' and F12='''+sYear+'''' +' group by F04' );
  Qry1.First;
  ...
  iMBTax := 0;
  iMBRecTotal := 0;
  QryExec(Qry1,'select F04,Sum(F08) as STaxtotal,Sum(F10) as SRectotal from T2050 where F13=''退'' and F03='''+sCNo+''' and F12='''+sYear+'''' +' group by F04' );
  Qry1.First;
  while not Qry1.Eof do
  begin
    //按币别统计
