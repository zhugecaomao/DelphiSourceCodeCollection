...
procedure TFM_3040.FormCreate(Sender: TObject);
begin
  inherited;
...
procedure TFM_3040.BitBtn4Click(Sender: TObject);
begin
  inherited;
  Query1.Params[0].AsString := copy(SpinEdit1.Text,3,2)+combobox1.Text;
  Query1.Close;
...
procedure TFM_3040.BitBtn2Click(Sender: TObject);
begin
  inherited;
...
procedure TFM_3040.BitBtn3Click(Sender: TObject);
begin
  inherited;
...
procedure TFM_3040.SB6Click(Sender: TObject);
begin
  inherited;
  close;
end;

procedure TFM_3040.DBGrid1EditButtonClick(Sender: TObject);
begin
  inherited;
...
procedure TFM_3040.Table2BeforePost(DataSet: TDataSet);
begin
  inherited;
  ...
  begin
    QryExec(Qry1,'select Max(K_SN) as MaxK_SN from T3020A where F01='''+Table1F01.AsString+''' and F02='''+Table1F02.AsString+'''');
      if Qry1.Fields[0].IsNull then
        Table2K_SN.AsInteger := 1 else
          Table2K_SN.AsInteger := Qry1.Fields[0].asinteger+1;

    QryExec(Qry1,'select Max(D_SN) as MaxD_SN from T3020A where F01='''+Table1F01.AsString+''' and F02='''+Table1F02.AsString+'''') ;
      if  Qry1.Fields[0].IsNull then
...
procedure TFM_3040.Table2BeforeDelete(DataSet: TDataSet);
begin
  inherited;
  iSN := Table2D_SN.AsInteger;
end;

procedure TFM_3040.Table2AfterDelete(DataSet: TDataSet);
begin
  inherited;
  QryExec(Qry1,'select Max(D_SN) as MaxSN from T3020A');
  if iSN < Qry1.FieldByName('MaxSN').asinteger then
     Qry1.Close;
     Qry1.SQL.Clear;
     Qry1.SQL.Add('Update T3020A set D_SN=D_SN-1 where D_SN > '''+inttostr(iSN)+''''+' and F01='''+Table1F01.AsString+''' and F02='''+Table1F02.AsString+'''');
     Qry1.ExecSQL;
     Table2.Refresh;
  Table1.Edit;
  Table1.Post;
...
procedure TFM_3040.Table1CalcFields(DataSet: TDataSet);
var
  iDSum:real;
begin
  inherited;
  ...
  QryExec(Qry1,'select F04,sum(F10) as DSum from T3020A where F01='''+Table1F01.AsString+''' and F02='''+Table1F02.AsString+''' Group by F04');
  Qry1.First;
...
procedure TFM_3040.Table2AfterPost(DataSet: TDataSet);
begin
  inherited;
  Table1.Edit;
  Table1.Post;
...
procedure TFM_3040.Table1BeforePost(DataSet: TDataSet);
begin
  inherited;
...
procedure TFM_3040.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  inherited;
...
procedure TFM_3040.Table2CalcFields(DataSet: TDataSet);
begin
  inherited;
  Table2CalF10.AsFloat := Table2F06.AsFloat + Table2F07.AsFloat;
end;

procedure TFM_3040.Panel4Exit(Sender: TObject);
begin
  inherited;
  BitBtn3.click;
end;

procedure TFM_3040.Panel5Exit(Sender: TObject);
begin
  inherited;
...
procedure TFM_3040.Table2BeforeInsert(DataSet: TDataSet);
begin
  inherited;
...
procedure TFM_3040.Table1AfterScroll(DataSet: TDataSet);
begin
  inherited;
...
procedure TFM_3040.Table1AfterEdit(DataSet: TDataSet);
begin
  inherited;
  BitBtn2.Enabled := true;
  BitBtn3.Enabled := true;
...
procedure TFM_3040.Table1F06Validate(Sender: TField);
begin
  inherited;
  EValue_IsNotNull(sender,table1F06.AsString);
end;

procedure TFM_3040.Table1F07Validate(Sender: TField);
begin
  inherited;
  EValue_IsNotNull(sender,table1F07.AsString);
end;

procedure TFM_3040.Table1F08Validate(Sender: TField);
begin
  inherited;
  EValue_IsNotNull(sender,table1F08.AsString);
end;

procedure TFM_3040.Table1F14Validate(Sender: TField);
begin
  inherited;
