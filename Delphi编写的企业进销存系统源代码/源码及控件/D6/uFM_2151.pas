...
type
  TFM_2151 = class(TS_MasterDetail_With_Item)
...
procedure TFM_2151.Table1BeforePost(DataSet: TDataSet);
begin
  inherited;
  ...
  //取得结帐日
  //DBDateEdit1Exit(DBDateEdit1);
  //RxDBComboEdit2Exit(RxDBComboEdit2)

end;

procedure TFM_2151.RxDBComboEdit1ButtonClick(Sender: TObject);
begin
  inherited;
  Select_Data(DataSource1,'人员点选窗口','select F01 as 姓名 from T1040','STK0311',['F04'],[0],[]);
end;

procedure TFM_2151.DBGrid1EditButtonClick(Sender: TObject);
var
  iAutoNo,iNo: integer;
  sNo,sNo1:string;
begin
  ...
  if DBGrid1.SelectedField = Table2F06 then  begin
    if Select_Data(DataSource2,'制令单选取窗口','select M.F01 as 制令单号,'
                ...
                +'left join T1080 as P ON M.F03=P.AutoNo where M.F07<M.F06 and M.F09=''成品''','STK0311',['F06','F02','F03','F10'],[0,2,3,4],[80,60,0])
    then begin
...
procedure TFM_2151.DetailEdit(DataSet: TDataSet);
begin
  //删除一笔明细,减去对应的数量
...
procedure TFM_2151.Table1CalcFields(DataSet: TDataSet);
begin
  inherited;
  DSum.Close;
  DSum.Open;
  ...
  begin
    Table1CalF08.asFloat :=  DealFractional(Table1F07.AsFloat * DSumDSum.AsFloat,2);
    Table1CalF10.AsFloat := DSumDSum.AsFloat;

  end;
  if Table1F06.AsString = '内含'  then
  begin
    Table1CalF08.AsFloat := DealFractional(Table1F07.AsFloat * DSumDSum.AsFloat,2);
    Table1CalF10.AsFloat := DealFractional(DSumDSum.AsFloat*(1-Table1F07.AsFloat),2);
  end;
...
procedure TFM_2151.Table2CalcFields(DataSet: TDataSet);
begin
  inherited;
  Table2CalF05.AsFloat := DealFractional(Table2F03.AsFloat * Table2F04.AsFloat,2);
end;

procedure TFM_2151.Table2BeforePost(DataSet: TDataSet);
begin
  inherited;
  //把建档人,修改人,建档时间,修改时间存入资料表;
  InforSave(DataSet);
...
procedure TFM_2151.Table2AfterPost(DataSet: TDataSet);
begin
  inherited;
  InforDisplay(DataSet,'成品委外入库单明细档');
  Table1.Edit;
  Table1.Post;  
end;

procedure TFM_2151.RxDBComboEdit2ButtonClick(Sender: TObject);
begin
  inherited;
...
procedure TFM_2151.Table1AfterScroll(DataSet: TDataSet);
begin
  inherited;
  if Table1.State in [dsInsert] then DBDateEdit1.Enabled := true
  else DBDateEdit1.Enabled := false;
  InforDisplay(DataSet,'成品委外入库单主档');
end;

procedure TFM_2151.Table1AfterPost(DataSet: TDataSet);
begin
  inherited;
...
procedure TFM_2151.Table2AfterScroll(DataSet: TDataSet);
begin
  inherited;
  InforDisplay(DataSet,'成品委外入库单明细档');
end;

procedure TFM_2151.RxDBComboEdit3ButtonClick(Sender: TObject);
begin
  inherited;
Select_Data(DataSource1,'币别点选窗口','select F01 as 币别,F02 AS 汇率 from T1100','STK0311',['F04'],[0],[]);
end;

procedure TFM_2151.Table1AfterInsert(DataSet: TDataSet);
begin
  inherited;
...
procedure TFM_2151.DBDateEdit1Exit(Sender: TObject);
begin
  inherited;
...
procedure TFM_2151.SB3Click(Sender: TObject);
begin
  inherited;
  Search_MasterDetailex(table1,[0,1,1,2],table2,[3,9,17],[0,1,2],[3,9,10,11,17,24],['T1030','T1080'],['F01','AutoNo'],['F03','F01,F02,F03,F04,F05,F07'],['厂商简称','型号,品名,材质,规格,心孔,单重'],['M.F03','D.F02'],'');
end;

procedure TFM_2151.SB2Click(Sender: TObject);
begin
  inherited;
  if Application.FindComponent('RP_2151')=nil then
    RP_2151 := TRP_2151.Create(Application);
  with Rp_2151 do
  begin
    Query1.Close;
...
procedure TFM_2151.Table2F20Change(Sender: TField);
begin
  inherited;
  lNeedCalWeigh := false;
  if lNeedCalQty then
...
procedure TFM_2151.Table2F03Change(Sender: TField);
begin
  inherited;
  LNeedCalQty := false;
...
procedure TFM_2151.Table1BeforeInsert(DataSet: TDataSet);
begin
  inherited;
  IfDo('新增',Qry1,GlobalUser,Hint,'F02');
end;

procedure TFM_2151.Table1BeforeEdit(DataSet: TDataSet);
begin
  inherited;
   if IfEdit = '0' then
...
procedure TFM_2151.Table1BeforeDelete(DataSet: TDataSet);
begin
  inherited;
  IfDo('删除',Qry1,GlobalUser,Hint,'F03');
end;

procedure TFM_2151.Table2BeforeDelete(DataSet: TDataSet);
begin
  inherited;
  IfDo('删除',Qry1,GlobalUser,Hint,'F03');
end;

procedure TFM_2151.Table2BeforeEdit(DataSet: TDataSet);
begin
  inherited;
...
procedure TFM_2151.Table2BeforeInsert(DataSet: TDataSet);
begin
  inherited;
   IfDo('新增',Qry1,GlobalUser,Hint,'F02');
end;
