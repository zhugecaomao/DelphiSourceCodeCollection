...
begin
  ...
  if DBGrid1.SelectedField = Table2F06 then  begin
    if Select_Data(DataSource2,'制令单选取窗口','select F01 as 制令单号,F02 as 需求物品编号,F03 as 型号,F04 as 品名,F05 as 规格,F06 as 材质,F07 as 心孔,F10-F11 as 可领量 from T2020A where F08=''半成品'' and F11<F10','STK0311',['F06','F02','F03'],[0,1,7],[80,0])
    then begin
...
//显示资料新增与修改记录
  InforDisplay(Dataset,'半成品委外领(退)料主档');
end;
...
begin
  inherited;
//显示资料新增与修改记录
  InforDisplay(Dataset,'半成品委外领(退)料明细档');
end;
...
begin
  inherited;
  Search_MasterDetailex(table1,[0,1,1,4,6],table2,[7,5],[0,1,4,6],[17,7,5],['T1030','T1070'],['F01','AutoNo'],['F03','F01,F02,F03,F04,F05,F07'],['厂商简称','型号,品名,材质,规格,心孔,单重'],['M.F03','D.F02'],'');
end;
...
begin
  ...
  begin
    ...
    Query1.Open;
    IF DBComboBox1.Text = '领' then
      QRLabel1.Caption := '半成品委外领料单'
    else
      QRLabel1.Caption := '半成品委外退料单';
    Preview;
  end;
end;
