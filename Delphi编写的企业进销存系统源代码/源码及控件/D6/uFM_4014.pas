...
uses uDM,utilities, uFM_4010A, uRp_4014A, uRp_4014B;

{$R *.DFM}

procedure TFM_4014.BitBtn1Click(Sender: TObject);
 var
   sName,sNo,NN: string;
   MM: Integer;
...
inherited;
  sName := GetSeleChMan;
   ...
   QryExec(Qry1,'Select Max(F01) from T4014');
   if Qry1.Fields[0].AsString = '' then
     ...
     with Qry1 do
      begin
       Close;
       Sql.Clear;
       Sql.Add('Insert into T4014(F01,F02,F03) select :MNo,:MDate,:MMan');
       ...
       QryExec(Qry1,'Select AutoNo,F06,F10,F11 from T1090');
       while not Qry1.Eof do
        begin
         Qry2.Close;
         Qry2.SQL.Clear;

         Qry2.SQL.Add('Insert into T4014A(F01,F02,F03,F07,F08) select :DNo,:PNo,:PQty,:F07,:F08');
         ...
         Qry2.Params[4].AsString := Qry1.Fields[3].AsString;
         Qry2.ExecSQL;
...
procedure TFM_4014.BitBtn2Click(Sender: TObject);
var
  Qty: real;
begin
  inherited;
  QryExec(Qry1,'select F04 from T4014A where (F04='''' or F04 IS Null) and F01='''+Table1F01.AsString+'''');
  ...
  Try
    ...
    begin
      QryExec(Qry2,'Update T1090 set F06='+Table2F04.AsString+' where AutoNo='+Table2F02.AsString,False);

      QryExec(Qry2,'select F02,F10-F11 as Qty from T2020A where F08="客户来料" and F02="'+Table2F02.AsString+'"');
      Qty := Qry2.Fields[1].asfloat;
      QryExec(Qry2,'Update T1090 set F12=' +Floattostr(Table2F04.asfloat-Qty)+' where AutoNo="'+Table2F02.AsString+'"',false);
...
procedure TFM_4014.FormShow(Sender: TObject);
begin
  inherited;
  QryExec(Qry1,'Select Max(F01) from T4014');
...
procedure TFM_4014.FormCreate(Sender: TObject);
begin
  inherited;
  Table1.Active := True;
  Table2.Active := True;
...
procedure TFM_4014.Table1AfterDelete(DataSet: TDataSet);
begin
   Qry1.Close;
   Qry1.SQL.Clear;
   Qry1.SQL.Add('Delete from T4014A where F01='''+ sEditTxt + '''');
   Qry1.ExecSQL;
   Table2.Refresh;
...
procedure TFM_4014.Table1BeforeDelete(DataSet: TDataSet);
begin
...
procedure TFM_4014.BitBtn5Click(Sender: TObject);
begin
  inherited;
  Close;
end;

procedure TFM_4014.Table2CalcFields(DataSet: TDataSet);
begin
  inherited;
...
procedure TFM_4014.Table2BeforePost(DataSet: TDataSet);
begin
  inherited;
...
procedure TFM_4014.BitBtn3Click(Sender: TObject);
begin
  inherited;
  if Application.FindComponent('Rp_4014A')=nil then
     Rp_4014A := TRp_4014A.Create(self);
  with Rp_4014A do
   begin
    Query1.Close;
...
procedure TFM_4014.BitBtn4Click(Sender: TObject);
begin
  inherited;
  if Application.FindComponent('Rp_4014B')=nil then
     Rp_4014B := TRp_4014B.Create(self);
  with Rp_4014B do
   begin
    Query1.Close;
    Query1.Params[0].AsString := DBEdit1.Text;
...
procedure TFM_4014.BitBtn6Click(Sender: TObject);
var
 xs: Tstrings;
begin
  xs := Select_text('选择客户来料编号','select AutoNo as ''编号'', F01 as ''型号'',F02 as ''品名'',F03 as ''材质'', F04 as ''规格'', F05 as ''心孔'' from T1090 order by f01','STK0311',[0,1,2,3,4,5],[]);
  if Assigned(xs) then
  try
...
procedure TFM_4014.Table1AfterScroll(DataSet: TDataSet);
begin
  inherited;
   //设置更新按钮致能,数量可否更改
...
procedure TFM_4014.Table2BeforeInsert(DataSet: TDataSet);
begin
  inherited;
  Abort;
end;

procedure TFM_4014.Table2BeforeDelete(DataSet: TDataSet);
begin
  inherited;
  Abort;
end;
