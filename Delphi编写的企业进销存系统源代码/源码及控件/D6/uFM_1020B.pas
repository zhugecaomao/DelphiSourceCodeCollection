...
procedure TFM_1020B.BitBtn2Click(Sender: TObject);
begin
  inherited;
   Close;
end;

procedure TFM_1020B.ComboEdit1ButtonClick(Sender: TObject);
var
 xs : TStringList;
begin
   xs := Select_text('选择品名','select distinct F03 as 品名 from T1020A ','STK0311',[0],[]);
   if Assigned(xs) then
   try
...
procedure TFM_1020B.ComboEdit2ButtonClick(Sender: TObject);
var
 xs : TStringList;
begin
   xs := Select_text('选择规格','select distinct F04 as 规格 from T1020A ','STK0311',[0],[]);
   if Assigned(xs) then
   try
...
procedure TFM_1020B.BitBtn1Click(Sender: TObject);
begin
  inherited;
  Query1.Close;
  Query1.SQL.Clear;
  Query1.SQL.add('select F01,K_SN,F02,F03,F04,F05,F06,F07 from T1020A where 0=0');
