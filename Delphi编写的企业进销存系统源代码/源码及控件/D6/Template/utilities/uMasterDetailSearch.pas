...
procedure TMasterDetailSearch.SetSearchGrid;
var
  iCount: integer;
  iRows,xx: integer;
begin
  ...
  for iCount := low(FLookUpData) to high(FLookUpData) do
      xx := xx +SubStrCnt(',',FLookupVisibleFields[iCount])+1;
  PubFieCount := xx;
  iRows := FMFieldCount+FDFieldCount+xx;
...
procedure TMasterDetailSearch.tsGrid1ButtonClick(Sender: TObject; DataCol,
  DataRow: Integer);
begin
...
procedure TMasterDetailSearch.StartSearch(Sender: TObject);
var
  sWhere: string;
  ...
  var
    i: integer;
  begin
    for i := 0 to (FMFieldCount+FMFieldCount+PubFieCount)-1 do
...
procedure TMasterDetailSearch.tsGrid1ComboGetValue(Sender: TObject;
...
procedure TMasterDetailSearch.tsGrid1ComboDropDown(Sender: TObject;
...
procedure TMasterDetailSearch.BitBtn3Click(Sender: TObject);
begin
  Close;
end;

procedure TMasterDetailSearch.BitBtn2Click(Sender: TObject);
var
  sParimayKey: string;
  icount: Integer;
  vFieldArray: variant;
...
procedure TMasterDetailSearch.tsDBGrid1DblClick(Sender: TObject);
begin
  BitBtn2Click(nil);
end;

procedure TMasterDetailSearch.Query1AfterOpen(DataSet: TDataSet);
begin
...
procedure TMasterDetailSearch.tsGrid1CellEdit(Sender: TObject; DataCol,
...
procedure TMasterDetailSearch.BitBtn4Click(Sender: TObject);
begin
  GetSelRang;
  SearchRepExc(Query1,FSelR,FSelRang);
end;

procedure TMasterDetailSearch.BitBtn5Click(Sender: TObject);
begin
  GetSelRang;
  ExcelPrint(Query1,'Excel.Application','资料表',FSelR,FSelRang);
end;

procedure TMasterDetailSearch.FormDestroy(Sender: TObject);
begin
  FKeyValue.Free;
end;

procedure TMasterDetailSearch.FormCreate(Sender: TObject);
begin
  FormResizer1.InitializeForm;
end;

procedure TMasterDetailSearch.FormResize(Sender: TObject);
begin
