...
uses
  ...
  CSCustomdxDateEdit, CSdxDBDateEdit, dxmdaset, ufrmPersonMng, ufrmOrganMng,
...
procedure TfrmPsninfract.lcmbOrganEditButtons0Click(Sender: TObject;
  var Handled: Boolean);
begin
  prip_CompanyDialog((SUPERVISOR_SIGN or VERIFY_SIGN), lcmbOrgan, TfrmOrganMng.Create(nil));
end;
...
begin
  ...
  begin
    if Active then Close;
    SQL.Text := Format('SELECT * FROM device.person_infract_info WHERE ' +
