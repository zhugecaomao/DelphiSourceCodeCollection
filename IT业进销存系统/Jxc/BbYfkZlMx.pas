...
procedure TfrmBbYfkZlMx.FormShow(Sender: TObject);
var
  i, b, e: integer;
begin
...
procedure TfrmBbYfkZlMx.IniRecord;
var
  strSum: string;
begin
  ...
  begin
    CommandText :=
      ...
      'from (' + GetSqlYfmx(dteRQ, intGYSID) + ') M';
    Open;
  ...
  dsMx.CommandText :=
    ...
    'M.JE as JEA, M.YFJE as JEB, M.JE1 as JEC ' +
    'from (' + GetSqlYfmx(dteRQ, intGYSID) + ') as M ' +
    'order by M.RQ, M.DJBH';
  inherited;
end;

procedure TfrmBbYfkZlMx.aDjExecute(Sender: TObject);
begin
  OpenDj('挂帐采购', dsMx['ZBID']);
end;

procedure TfrmBbYfkZlMx.SetButton;
begin
  inherited;
