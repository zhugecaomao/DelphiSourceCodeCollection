...
procedure TfrmBbDqXsMx.FormShow(Sender: TObject);
begin
...
procedure TfrmBbDqXsMx.IniRecord;
begin
  dsMx.CommandText :=
    ...
    'JEA + JEB as JEC ' +
    'from (' + GetSqlXsmx(dteRQA, dteRQB, Format('DQMC = ''%s''', [strMC])) + ') M, KH K ' +
    'where K.ID = M.KHID ' +
...
procedure TfrmBbDqXsMx.aDjExecute(Sender: TObject);
begin
  OpenDj(dsMx['LX'], dsMx['ZBID']);
end;

procedure TfrmBbDqXsMx.SetButton;
begin
  inherited;
  aDj.Enabled := (aDj.Tag = 0) and not (dsMx.IsEmpty);
end;

procedure TfrmBbDqXsMx.DBGrid1DrawColumnCell(Sender: TObject;
