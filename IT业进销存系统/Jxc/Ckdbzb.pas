...
var
  frmCkdbzb: TfrmCkdbzb;

implementation

uses Dm, Main, ComDj, CKDB, ComFun;

{$R *.DFM}

procedure TfrmCkdbzb.FormShow(Sender: TObject);
begin
  strAutoScale := 'DBGrid2';
  inherited;
end;

procedure TfrmCkdbzb.LoadEditForm(strState: string);
begin
  frmDj := TfrmComDj(TfrmCkdb.Create(Application));
  inherited;
end;

procedure TfrmCkdbzb.OpendataSet;
var
  strWhere: string;
begin
  //同时更新 MyLib.Open
  ...
  dsJbzl.CommandText :=
    ...
    'Z inner join DR on Z.DRCID = DR.ID ' + strWhere + ' ' +
    'order by Z.DJBH';
  inherited;
end;

procedure TfrmCkdbzb.dsAfterScroll(DataSet: TDataSet);
begin
  inherited;
