...
type
  TfrmCgshHelp = class(TfrmComHelp)
    DBGrid2: TDBGrid;
...
function TfrmCgshHelp.GetFilter: string;
begin
  result := 'RQ >= #' + FormatDateTime('yyyy-mm-dd', dtpRqa.Date) + '# and ' +
    'RQ <= #' + FormatDateTime('yyyy-mm-dd', dtpRqb.Date) + '#';
end;

procedure TfrmCgshHelp.dsAfterScroll(DataSet: TDataSet);
begin
  inherited;
...
procedure TfrmCgshHelp.btnOkClick(Sender: TObject);
begin
...
procedure TfrmCgshHelp.DBGrid2DrawColumnCell(Sender: TObject;
