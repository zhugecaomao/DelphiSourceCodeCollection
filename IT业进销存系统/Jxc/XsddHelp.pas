...
type
  TfrmXsddHelp = class(TfrmComHelp)
    DBGrid2: TDBGrid;
...
function TfrmXsddHelp.GetFilter: string;
begin
  result := 'RQ >= #' + FormatDateTime('yyyy-mm-dd', dtpRqa.Date) + '# and ' +
    'RQ <= #' + FormatDateTime('yyyy-mm-dd', dtpRqb.Date) + '#';
end;

procedure TfrmXsddHelp.dsAfterScroll(DataSet: TDataSet);
begin
  inherited;
...
procedure TfrmXsddHelp.btnOkClick(Sender: TObject);
begin
  if (btnOk.Enabled) and (dsEditing <> nil) then
    ...
    begin
      ...
      while not Eof do
      begin
        if FieldByName('SL').AsFloat > FieldByName('aYTSL').AsFloat then
        begin
          dsEditing.Append;
          dsEditing.FieldByName('aBH').AsString :=
            FieldByName('aBH').AsString;
          dsEditing.FieldByName('SL').AsFloat :=
            FieldByName('SL').AsFloat - FieldByName('aYTSL').AsFloat;
...
procedure TfrmXsddHelp.DBGrid2DrawColumnCell(Sender: TObject;
