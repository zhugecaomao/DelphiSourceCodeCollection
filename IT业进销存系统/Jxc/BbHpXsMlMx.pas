...
type
  TfrmBbHpXsMlMx = class(TfrmComMx)
    Label4: TLabel;
    Label5: TLabel;
    labHPBH: TLabel;
    labHPPM: TLabel;
...
procedure TfrmBbHpXsMlMx.FormShow(Sender: TObject);
begin
...
procedure TfrmBbHpXsMlMx.IniRecord;
begin
  dsMx.CommandText :=
    'select M.LX, M.ZBID, M.RQ, M.DJBH, K.BH, K.MC, M.SL1 as SL, M.DJ, M.CBDJ, M.NSL, ' +
...
procedure TfrmBbHpXsMlMx.aDjExecute(Sender: TObject);
begin
  OpenDj(dsMx['LX'], dsMx['ZBID']);
end;

procedure TfrmBbHpXsMlMx.SetButton;
begin
  inherited;
  aDj.Enabled := (aDj.Tag = 0) and not (dsMx.IsEmpty);
end;

procedure TfrmBbHpXsMlMx.DBGrid1DrawColumnCell(Sender: TObject;
