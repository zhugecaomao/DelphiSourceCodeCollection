  ...
  begin
    ...
    begin
      ...
      if dblZKCL - Field.AsFloat < 0 then
        Application.MessageBox(Pchar('退货后，货品''' + VarToStr(dsMx['aPM']) + '''的总库存量将出现负数。'), '提醒你', MB_OK + MB_ICONWARNING)
      else
      begin
        dblXX := VarToFloat(GetFieldValue('select XX from HP where ID = ' + Float2Str(dsMx['HPID'])));
        if dblXX <> 0 then
          if dblZKCL - Field.AsFloat < dblXX then
            Application.MessageBox(Pchar('退货后，货品''' + VarToStr(dsMx['aPM']) + '''的总库存量已经低于设定的库存下限。'), '提醒你', MB_OK + MB_ICONWARNING);
      end;
    end;
  end;
end;
...
begin
  ...
  begin
    Application.MessageBox('''收款方式''或''收款帐户''未录入, 请继续录入!', '录入错误', MB_OK + MB_ICONWARNING);
