...
begin
  ...
  begin
    Application.MessageBox(Pchar('可用总金额不能为零, 请输入''付款金额''或选择''使用结余额''!'), '录入错误', MB_OK + MB_ICONWARNING);
    abort;
  end;
  if (dsZb.FieldByName('FKLX').AsInteger = 1) and (Str2Float(labJED.Caption) = 0) then
  begin
    Application.MessageBox(Pchar('付款金额没有付任何应付单据。（如果要进行预付付款，请选择''付款类型''为预付款）'), '录入错误', MB_OK + MB_ICONWARNING);
    abort;
  end;
  if (dsZb.FieldByName('FKLX').AsInteger = 1) and
    (Str2Float(labJED.Caption) > Str2Float(labKYZJE.Caption)) then
  begin
    Application.MessageBox(Pchar('本次付款总金额不应该大于可用总金额，请重新分配本次付款。'), '录入错误', MB_OK + MB_ICONWARNING);
    abort;
  end;
  blnErr := False;
  with dsMx do
  ...
  if blnErr then
  begin
    Application.MessageBox(Pchar('本次付款不应该大于收货单末付金额。'), '录入错误', MB_OK + MB_ICONWARNING);
    abort;
  end;
  if (dsZb.FieldByName('FKLX').AsInteger = 1) and
    (Str2Float(labJED.Caption) < Str2Float(labKYZJE.Caption)) then
    if Application.MessageBox( '付款有结余，结余金额转入对应供应商预付款中?',
