...
procedure TfrmFykz.FormShow(Sender: TObject);
begin
...
procedure TfrmFykz.ZbOnNewRecord(DataSet: TDataSet);
begin
  inherited;
  DataSet['RQ'] := Date;
...
procedure TfrmFykz.sZbDataChange(Sender: TObject; Field: TField);
begin
  inherited;
  if (Field <> nil) and (TDataSource(Sender).State in [dsInsert, dsEdit]) then
  begin
    if (UpperCase(Field.FieldName) = 'RQ') then
      dsZb['DJBH'] := GetDjbh('KZ', Field.AsDateTime, 'FYKZZB');
...
procedure TfrmFykz.TotalField(Kind: char; Field: TField);
var
...
procedure TfrmFykz.sMxDataChange(Sender: TObject; Field: TField);
begin
  inherited;
  if (Field <> nil) and (TDataSource(Sender).State in [Dsinsert,Dsedit]) then
  begin
    if (UpperCase(Field.FieldName) = 'ABH') and (Field.AsString <> '') then
      with CurDs do
      begin
        CommandText := 'select * from SRZCLB where not SRL and BH = ''' + Field.AsString + '''';
        Open;
        if not IsEmpty then
        begin
...
procedure TfrmFykz.DBGrid1EditButtonClick(Sender: TObject);
begin
